...
class GridClass {
    ...
    if (Game.phase != 'Game is done') {
      ...
      for (var y = 0; y < this.grid.length; y++) {
        for (var x = 0; x < this.grid[y].length; x++) {
          ...
          for (
            var i = 0;
            i < this.calculateNeighboringcells(x, y).length;
            i++
          ) {
  ...
  saveGrid() {
    ...
    for (var y = 0; y < this.grid.length; y++) {
      for (var x = 0; x < this.grid[y].length; x++) {
        if (this.grid[y][x].color == 'black') {
          coloredCell.push([this.grid[y][x].X, this.grid[y][x].Y]);
        }
      }
    }
    var json = {
    ...
    saveAs(
      blob,
      prompt('Entrez le nom sous lequel vous voulez sauvegarder la grille'),
    );
  }
}
...
$('#jsonGrid').click(function () {
  let json = JSON.parse(
    prompt(
      "Collez le JSON d'une grille que vous avez préalablement téléchargé",
    ),
  );
  Grid = new GridClass(json.nbX, json.nbY);
  Grid.updateCells(json.coloredCell);
